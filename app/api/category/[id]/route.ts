    ...
    try {
        ...
        if (dataValidate.error) {
            return  NextResponse.json({msg: dataValidate.error.message,  status: 422, ok:false });
             ...
             const productCategory = await prismadb.category.update({
            where: { id: Number(id) },
...
export async function DELETE(req: Request, { params  }: { params: {id: string}}) {
    try {
        const id = Number(params.id);
        const productCategory = await prismadb.category.delete({
            where: { id: id }
        });
...
export async function GET(req: Request, { params  }: { params: {id: string}}) {
    try {
        const id = Number(params.id);
        const productCategory = await prismadb.category.findUnique({
            where: { id }
        });

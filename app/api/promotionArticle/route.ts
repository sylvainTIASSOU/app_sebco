...
export async function POST(req: Request) {
    try {
        ...
        const dataValidation = Dto.promotionProductDto().validate(promotionArticleModel);
        if (dataValidation.error) {
            return  NextResponse.json({msg: dataValidation.error.message,  status: 400, ok:false});
        } else {
            const promotionArticle = await prismadb.promotionProduct.create({ data: dataValidation.value });
...
export async function GET() {
    try {
        const promotionArticle = await prismadb.promotionProduct.findMany({
            include: {
                promotion: true,
                product:{
                    include: {

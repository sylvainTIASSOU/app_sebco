...
import {OrderModel} from "@/models/orderModule/OrderModel";
import {OrderProductModel} from "@/models/orderModule/OrderProductModel";
import {DeliveryModel} from "@/models/orderModule/DeliveryModel";
   ...
   const {
    ...
    async function successHandler(response: any) {
        ...
        if (resp1.ok) {
            ...
            for (const ele of itemCart) {
                const orderArticle = new OrderProductModel(ele.quantity, ele.price, Number(ele.id), orderId);
    ...
    function open() {
        openKkiapayWidget({
            ...
            api_key: "eff95894c4bf706ff11ca8541c63da3bd59103d3",
            sandbox: false,
            email: `${customer?.email}`,
            phone: "97000000",
        });
    }
    //process.env.KKIAPAY_API_SANDBOX,
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setIsLoading(true)
            if (isAuth) {
                const deliveryModel = new DeliveryModel(values.city, values.quarter, values.deliveryDate, values.deliveryHoures, values.indiqueName, Number(values.indiqueNumber), +uid, `code: ${values.codePromo}`);
                const resp = await Api.create(`/api/delivery`, deliveryModel );
                if (resp.ok) {
                    //setDeliveryId(resp.delivery.id);
                    console.log(resp.delivery.id)
                    setIsLoading(true);
                    const orderModel = new OrderModel(total, "NEW", "PAY", Number(resp.delivery.id));
                    const resp1 = await Api.create('/api/order', orderModel,);
                    if (resp1.ok) {
                        const orderId = resp1.order.id;
                        setOrder(resp1.order.id);

...
export default function Orders() {
    ...
    const tableConstruction = (data: any[]) => {
        ...
        return data.map((items) => (
            <TableRow key={items.id}>
                ...
                <TableCell
                    className={getOrderStatusColor(items.status)}>
                    <Select
                        placeholder={""}
                        ...
                        onSelect={async (value) => {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    const orderModel = new OrderModel(items.totalPrice, value, items.payement, +items.deliveryId, +items.id);
    ...
    return (
        <main className={' px-0 flex flex-col space-y-10'}>
            <div className={'flex justify-between content-between'}>
                <div className={'flex flex-col space-y-0'}>
                    <h1 className={'font-extrabold text-[30px]'}>Commandes ({filteredData.length})</h1>
            ...
            <div className={"grid grid-cols-1 md:grid-cols-3 gap-4"}>
                ...
                <div className="flex items-center justify-center my-5">
                    <div className="flex space-x-3 items-center">
                        <Input type={"date"} className=" w-[250px] bg-white" placeholder={"Recherche par date de la commande"} value={query}

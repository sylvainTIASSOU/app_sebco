...
export default  function Customer() {
    ...
    useEffect(() => {
        ...
        Api.read(`/api/user/getUserByRole/CUSTOMER`).then((val) => {
            setData(val);
        })
    }, []);
    ...
    const tableConstruction = (data: UserModel[]) => {
        return data.map((items) => (
            <TableRow key={items.id}>
                ...
                <TableCell className={ "bg-lime-300"}>{items.role}</TableCell>
                {/*actions*/}
    ...
    return (
        <div className={"flex flex-col gap-10 w-full"}>
            <ContentListHeader title={"Client"} subtitle={"Gestion des utilisateur"} link={""} buttonTitle={""} count={data.length} />
            <div className={""}>

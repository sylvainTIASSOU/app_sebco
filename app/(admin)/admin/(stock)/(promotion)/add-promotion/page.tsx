...
export default function AddPromotion() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            const resp = await Api.create('/api/promotion', promotionModel);
            if(resp.ok) {
                const promotionArticleModel = new PromotionProductModel(+values.newPrice, +values.oldPrice, +resp.promotion.id, +values.productId);
                const resp2 = await Api.create('/api/promotionArticle', promotionArticleModel);
                if(resp2.ok) {
                    const resp3: ProductModel = await Api.read(`/api/article/${values.productId}`)
    ...
    return(
        <div className={"flex flex-col gap-10 "}>
            <ContendAddHeader subtitle={'Gestion du Stock'} title={'Ajouter une promotion'}/>

...
const OrderComp = ({status, title}: {status: string, title?: string}) => {
    ...
    const tableConstruction = (data: any[]) => {
        return data.map((items) => (
            <TableRow key={items.id}>
                ...
                <TableCell className="">
                    <Button variant={"outline"} size={"sm"} onClick={() => route.push(`/admin/edit-order/${items.id}`)}
        ...
        <main className={' flex flex-col space-y-10'}>
            <div className={'flex justify-between content-between'}>
                <div className={'flex flex-col space-y-0'}>
                    <h1 className={'font-regular text-[30px]'}>{title} ({filteredData.length})</h1>
